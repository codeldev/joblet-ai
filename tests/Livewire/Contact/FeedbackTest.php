...
use App\Livewire\Contact\Feedback;
use Illuminate\Support\Facades\DB;
use Livewire\Livewire;

describe(description: 'Feedback livewire component', tests: function (): void
{
    it('can render the feedback model component', function (): void
    {
        Livewire::test(name: Feedback::class)
            ->assertSee(values: trans(key: 'messages.feedback.title'));
    });

    it('populates form fields when a user is logged in', function (): void
    {
        Livewire::actingAs(user: $user = testUser())
            ->test(name: Feedback::class)
    ...
    it('successfully submits the form and sends a message', function (): void
    {
        Livewire::actingAs(user: $user = testUser())
            ...
            ->call(method: 'submit')
            ->assertDispatched(event: 'modal-close', name: 'feedback-form')
            ->assertDispatched(
                event   : 'toast-show',
                duration: 3500,
                slots   : [
                    'text' => trans(key: 'messages.feedback.success'),
                ],
                dataset : [
                    'variant'  => 'success',
                ]
    ...
    it('shows error on failure to send message', function (): void
    {
        ...
        Livewire::actingAs(user: testUser())
            ...
            ->assertDispatched(
                ...
                duration: 3500,
                slots   : [
                    'text' => trans(key: 'messages.feedback.failed'),
                ],
                dataset : [
                    'variant'  => 'danger',
                ]
